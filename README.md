...
This repository automates logical backups of Azure Database for MySQL databases for long-term retention. An Azure Automation runbook starts an Azure Container Instance that runs `mysqldump` and writes the resulting SQL dump to an Azure Files share.
...
- A geo-redundant storage account, Azure Files share, and private endpoint
...
At the scheduled time, the runbook creates a container group in the supplied delegated subnet. The container mounts the file share, runs `mysqldump` for the configured databases, and stops after the backup completes. Backup files are named `dumps-<timestamp>.sql`.
...
- You have a separate subnet available for the Azure Files private endpoint.
...
| `privateEndpointSubnetResourceId` | string | Yes | - | Resource ID of the subnet where the Azure Files private endpoint is created. |
